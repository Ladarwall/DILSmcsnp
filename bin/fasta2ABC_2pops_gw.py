...
bpfile_L1 = '# spA={0} spB={1} r={2}'.format(nameA, nameB, recomb)
bpfile_L2 = [] # L
bpfile_L3 = [] # nA
bpfile_L4 = [] # nB
bpfile_L5 = [] # nSNPs
...
if nameOut == 'NA':
	...
	if region == 'coding':
		...
		outfile_info = open('{0}/infos.txt'.format(timeStamp), 'w')
		...
		for locus_i in align['L'].keys():
			...
			if nSites >= Lmin: # if the locus is big enough to be considered
				...
				if nSynSegSite != 0:
					...
					bpfile_L3.append(nA)
					bpfile_L4.append(nB)
					bpfile_L5.append(nSynSegSite)
					bpfile_L6.append(4*Nref*nSites*recomb)
		...
		for locus_i in align['L'].keys():
			...
			if nSites >= Lmin: # if the locus is big enough to be considered
				if nSegSite != 0:
					...
					bpfile_L3.append(nA)
					bpfile_L4.append(nB)
					bpfile_L5.append(nSegSite) # theta = 4.Nref.mu.L, where Nref is the diploid population size
					bpfile_L6.append(4*Nref*recomb*nSites)
			...
			if locus_i in consensus:
				...
				if nSites >= Lmin: # if the locus is big enough to be considered
					if nSynSegSite != 0:
						...
						bpfile_L3.append(nA)
						bpfile_L4.append(nB)
						bpfile_L5.append(nSNPs) # theta = 4.Nref.mu.L, where Nref is the diploid population size
						bpfile_L6.append(4*Nref*recomb*nSites)
